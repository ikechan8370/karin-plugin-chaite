    ...
    async setItem(userId, userState) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.db.get(`SELECT userId FROM ${this.tableName} WHERE userId = ?`, [userId], (err, row) => {
                ...
                if (row) {
                    // 更新现有用户状态
                    this.db.run(`UPDATE ${this.tableName} SET 
              nickname = ?, 
              card = ?, 
              conversations = ?, 
              settings = ?, 
              current = ?, 
              updatedAt = ?
            WHERE userId = ?`, [nickname, card, conversationsJson, settingsJson, currentJson, updatedAt, userId], (err) => {
                        if (err) {
                            return reject(err);
                        }
                        resolve(userId);
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [crypto.randomUUID(), userId, nickname, card, conversationsJson, settingsJson, currentJson, updatedAt], (err) => {
                        if (err) {
                            return reject(err);
                        }
                        resolve(userId);
