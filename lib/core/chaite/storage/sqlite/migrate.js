...
export async function checkMigrate() {
    ...
    try {
        ...
        for (const pair of storagePairs) {
            ...
            if (items.length > 0) {
                ...
                for (const item of items) {
                    if (pair.name === '用户状态') {
                        await sqliteStorage.setItem(item.userId + '', item);
                    }
                    else {
        ...
        if (fs.existsSync(ChatGPTStorage.filePath)) {
            fs.copyFileSync(ChatGPTStorage.filePath, path.join(backupDir, `storage-backup-${timestamp}.json`));
            // 清空数据但保留文件结构
            for (const pair of storagePairs) {
                if (!pair.collection)
                    continue;
        ...
        if (fs.existsSync(ChatGPTHistoryStorage.filePath)) {
            fs.copyFileSync(ChatGPTHistoryStorage.filePath, path.join(backupDir, `history-backup-${timestamp}.json`));
            // 清空历史数据
