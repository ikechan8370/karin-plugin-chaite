...
import { logger, createRawMessage } from 'node-karin';
export class GroupContextCollector {
    async collect(e, groupId, start, length) {
...
export class KarinGroupContextCollector extends GroupContextCollector {
     ...
     */
    async collect(e, groupId, start = '', length = 20) {
...
export async function getGroupContextPrompt(e, length) {
    const { groupContextTemplatePrefix = '', groupContextTemplateMessage = '', groupContextTemplateSuffix = '' } = config().llm;
