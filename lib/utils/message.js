...
export async function intoUserMessage(e, options = {}) {
    const { handleReplyText = false, handleReplyImage = true, handleReplyFile = true, useRawMessage = false, handleAtMsg = true, excludeAtBot = false, toggleMode = 'at', togglePrefix = null } = options;
    const contents = [];
    let text = '';
    ...
    for (let element of e.elements?.filter((element) => element.type === 'image') || []) {
 ...
 */
export async function getPreset(e, // 假设 e 的类型未知，可以根据实际情况替换为具体类型
presetId, toggleMode, togglePrefix) {
 ...
 */
export function checkChatMsg(e, // 假设 e 的类型未知，可以根据实际情况替换为具体类型
toggleMode, togglePrefix) {
 ...
 */
export async function toYunzai(e, // 假设 e 的类型未知，可以根据实际情况替换为具体类型
contents) {
    ...
    for (let content of contents) {
        switch (content.type) {
            case 'text': {
                msgs.push(content.text?.trim() || '');
                break;
            }
            case 'image': {
                msgs.push(segment.image(content.image));
                break;
            }
            case 'audio': {
                msgs.push(segment.record(content.data));
                break;
            }
            case 'reasoning': {
                const reasoning = await common.makeForwardMsg(e, [content.text], '思考过程');
