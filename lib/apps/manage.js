...
const getManagerByName = (name) => {
    ...
    switch (name.toLowerCase()) {
        case 'channels': return chaite.getChannelsManager();
        case 'presets': return chaite.getChatPresetManager();
        case 'processors': return chaite.getProcessorsManager();
        case 'tools': return chaite.getToolsManager();
        default: return null;
    }
};
...
karin.command(new RegExp(`^(${config().basic.commandPrefix})?#?结束(全部)?对话$`), async (e) => {
    const chaite = await getChaiteInstance();
...
export const switchPresetCustomization = karin.command(new RegExp(`^${config().basic.commandPrefix}(允许|禁止)(预设切换|其他人切换预设)$`), async (e) => {
    if (!e.isMaster) {
        await e.reply('仅限主人使用');
        return;
    }
...
export const switchDebugMode = karin.command(new RegExp(`^${config().basic.commandPrefix}(开启|关闭)(调试|debug)(模式)?$`), async (e) => {
    if (!e.isMaster) {
        await e.reply('仅限主人使用');
        return;
    }
...
export const switchBymMode = karin.command(new RegExp(`^${config().basic.commandPrefix}(开启|关闭)(伪人|bym)$`), async (e) => {
    if (!e.isMaster) {
        await e.reply('仅限主人使用');
        return;
    }
