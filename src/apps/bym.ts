...
import { Chaite, EventMessage, SendMessageOption } from 'chaite'
import { intoUserMessage, toKarin } from '@/utils/message'
import { getGroupContextPrompt } from '@/utils/group'
import { formatTimeToBeiJing } from '@/utils/common'
...
export const bym = hooks.empty(async (e, next) => {
  ...
  if (pluginConfig.bym.hit.find((keyword: string) => e.msg?.includes(keyword))) {
    prob = 1
  }
  if (Math.random() > prob) {
    next() // 未触发伪人模式，继续后续处理
  ...
  if (pluginConfig.bym.presetMap && pluginConfig.bym.presetMap.length > 0) {
    const option = pluginConfig.bym.presetMap
      ...
      .find((item: { keywords: string[]; presetId: string; recall?: boolean }) =>
        item.keywords.find((keyword: string) => e.msg?.includes(keyword))
      )
    if (option) {
  ...
  // 复制预设的发送消息选项
  const sendMessageOption: SendMessageOption = JSON.parse(JSON.stringify(preset.sendMessageOption))
  ...
  // 设置多轮调用回调
  sendMessageOption.onMessageWithToolCall = async (content: any) => {
  ...
  if (pluginConfig.llm.enableGroupContext && e.isGroup) {
    try {
      ...
      logger.error('[BYM Mode]获取群聊上下文失败，本次对话将无视群聊上下文', err)
    }
  }

  // 发送消息并获取响应
  const response = await Chaite.getInstance().sendMessage(userMessage, e as unknown as EventMessage, Object.assign(sendMessageOption, { chatPreset: preset }))
